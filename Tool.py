...
class ExcelTool(object):
    ...
    def addItemToExcel(self, item, file_name=__file_name, sheet_name=__sheet_name):
        ...
        if os.path.exists(workbook_file_path):
            ...
            # 确保 item 的长度足够，至少为 15 列
            if len(item) < 15:
                item = list(item) + [""] * (15 - len(item))  # 补充缺失的列为空字符串
...
class TimeTaskModel:
    ...
    def __init__(self, item, msg:ChatMessage, isNeedFormat: bool, isNeedCalculateCron = False):
        ...
        # 延时参数
        self.delay_time = item[14] if len(item) > 14 else ""
    ...
    def is_recurring_task(self):
        ...
        """
        # 每天
        if "每天" in self.circleTimeStr:
            return True
        
        # 每周X
        if "每周" in self.circleTimeStr or "每星期" in self.circleTimeStr:
            return True
        
        # 工作日
        if "工作日" in self.circleTimeStr:
            return True
        
        # 每月X日
        if "每月" in self.circleTimeStr and "日" in self.circleTimeStr:
            return True
    ...
    def isPerson_makeGrop(self):
        ...
        tempValue1 = "group[" in self.eventStr or "Group[" in self.eventStr or "user[" in self.eventStr or "User[" in self.eventStr
    ...
    def get_Persion_makeGropTitle_eventStr(self):
        ...
        if "group[" in targetStr:
            ...
            index = targetStr.index("Group[")
        elif "user[" in targetStr:
            index = targetStr.index("user[")
        elif "User[" in targetStr:
            index = targetStr.index("User[")
        if index < 0:
              return "", targetStr
        ...
        substring_groupTitle = targetStr[index + 5:]
