...
class timetask(Plugin):
    ...
    def add_timeTask(self, content, e_context: EventContext):
        ...
        # 解析延时参数
        delay_time = ""
        if "t[" in eventStr.lower():
            delay_index = eventStr.lower().find("t[")
            delay_end_index = eventStr.find("]", delay_index)
            if delay_end_index != -1:
                delay_time = eventStr[delay_index + 2:delay_end_index]
                eventStr = eventStr[:delay_index].strip()
        ...
        user_name = ""
        group_title = ""
        if "[" in eventStr and "]" in eventStr:
            if "user[" in eventStr:
                user_name = eventStr[eventStr.find("user[")+5:eventStr.find("]")]
                eventStr = eventStr[:eventStr.find("user[")] + eventStr[eventStr.find("]")+1:]
            elif "group[" in eventStr:
                group_title = eventStr[eventStr.find("group[")+6:eventStr.find("]")]
                eventStr = eventStr[:eventStr.find("group[")] + eventStr[eventStr.find("]")+1:]
        ...
        taskInfo = ("",
                    ...
                    self.get_user_id_by_name(user_name) if user_name else "",
                    "1" if group_title else "0",
                    str(msg),
                    "0",
                    delay_time)  # 将延时参数存储到第15列
        ...
        # 指定用户名
        if user_name:
            taskModel.other_user_nickname = user_name
            taskModel.other_user_id = self.get_user_id_by_name(user_name)
        ...
        if len(taskId) > 0:
            ...
            # 添加延时参数提示
            if delay_time:
                reply_text += f"\n【延时参数】：任务将在 {delay_time} 分钟内随机延时执行。"
            # 添加温馨提示
            if taskModel.is_recurring_task():
                reply_text += "\n【温馨提示】：这是一个轮询任务，任务会按照设定的周期重复执行。"
            else:
                reply_text += "\n【温馨提示】：这是一个单次任务，任务将在指定时间执行一次。"
        else:
    ...
    def get_default_remind(self, currentType: TimeTaskRemindType):
        ...
        addTask += f"👉定群任务：{command_prefix} 今天 10:00 提醒我健身 user[用户名]" + "\n"
        addTask += f"👉延时任务：{command_prefix} 今天 10:00 提醒我健身 t[3-5]（任务将在 3-5 分钟内随机延时执行）" + "\n"
    ...
    def get_help_text(self, **kwargs):
        ...
        exampleStr += f"👉定用户任务：{command_prefix} 今天 10:00 提醒我健身 user[用户名]" + "\n"
        ...
        # 添加指定用户名的说明
        h_str3 = "🎉功能四：指定用户名\n"
        codeStr3 = f"【指令】：{command_prefix} 周期 时间 事件 user[用户名]\n"
        userNameStr = "【用户名】：指定接收消息的用户名，用户名需要用方括号 [] 包裹。\n"
        exampleStr3 = f"\n👉示例：{command_prefix} 今天 10:00 提醒我健身 user[用户名]\n\n\n"
        tempStr3 = h_str3 + codeStr3 + userNameStr + exampleStr3
        
        headStr = "📌 功能介绍：添加定时任务、取消定时任务、获取任务列表。\n\n"
        help_text = headStr + tempStr + tempStr1 + tempStr2 + tempStr3
        
        # 添加管理员认证提示
        help_text += "\n\n【温馨提示】：添加定时任务需要管理员认证，请输入 #auth [口令] 进行认证。"
