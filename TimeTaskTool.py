    ...
    def runTaskArray(self, modelArray):
        try:
            ...
            for _, model in enumerate(modelArray):
                # 检查是否有延时参数
                if hasattr(model, 'delay_time') and model.delay_time:
                    # 解析延时时间
                    delay_min, delay_max = map(int, model.delay_time.split('-'))
                    delay_minutes = random.randint(delay_min, delay_max)  # 随机生成延时分钟数
                    
                    # 获取当前任务的时间
                    current_time = arrow.get(model.timeStr, "HH:mm:ss")
                    # 在分钟上增加延时时间
                    new_time = current_time.shift(minutes=delay_minutes)
                    # 将秒数设置为 00
                    new_time = new_time.replace(second=0)
                    
                    # 判断是否为轮询任务
                    if model.is_recurring_task():  # 需要添加 is_recurring_task 方法
                        # 复制任务并调整时间
                        new_model = model.copy()
                        new_model.timeStr = new_time.format("HH:mm:ss")
                        new_model.delay_time = ""  # 新任务无延时参数
                        self.addTask(new_model)
                    else:
                        # 调整单次任务的时间
                        model.timeStr = new_time.format("HH:mm:ss")
                        model.delay_time = ""  # 删除延时参数
                        ExcelTool().write_columnValue_withTaskId_toExcel(model.taskId, 3, model.timeStr)  # 更新C列时间
                        ExcelTool().write_columnValue_withTaskId_toExcel(model.taskId, 15, "")  # 删除O列延时参数
